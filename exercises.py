...
def append_form_data_to_list():
    ...
    if submit_button:
        ...
        for looped_person in st.session_state.list_of_dict:
            st.write(looped_person)
    ...
    with st.status("Calling the OpenAI API..."):
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": "Tell me the story about Singapore in the 1970s in 50 words.",
                },
            ],
            temperature=0,
        )
...
def call_api_challenge():
    ...
    prompt_design = st.text_input(
        "Enter your the prompt design for the API call:",
        value="You are a helpful assistant.",
    )
    prompt_query = st.text_input(
        "Enter your prompt query:",
        value="Tell me about Singapore in the 1970s in 50 words.",
    )
    ...
    try:
        ...
        if prompt := st.chat_input("What is up?"):
            ...
            with st.chat_message("assistant"):
                ...
                for response in chat_completion_stream(
                    st.session_state.prompt_template + memory_context + rag, prompt
                ):
                    full_response += response.choices[0].delta.content or ""
                    message_placeholder.markdown(full_response + "▌")
                message_placeholder.markdown(full_response)
    ...
    try:
        ...
        if prompt := st.chat_input("What is up?"):
            ...
            with st.chat_message("assistant"):
                ...
                collect("me", full_response, prompt)
            st.session_state.chat_msg.append(
                {"role": "assistant", "content": full_response}
            )
